...
OUTPUT_FILE = ['/scratch/vaibhav/DPR-data/data/retriever/ocoqa-original.json',
               '/scratch/vaibhav/DPR-data/data/retriever/ocoqa-original-test.json']
OUTPUT_FILE_CSV = ['/scratch/vaibhav/DPR-data/data/retriever/qas/ocoqa-original.csv',
                   '/scratch/vaibhav/DPR-data/data/retriever/qas/ocoqa-original-test.csv']
GOLD_PASSAGE_INFO_FILE = ['/scratch/vaibhav/DPR-data/data/gold_passages_info/ocoqa-original.json',
                          '/scratch/vaibhav/DPR-data/data/gold_passages_info/ocoqa-original-test.json']
...
for idx in [0,1]:
    ...
    for conv in tqdm(convs):
        ...
        for i, turn in enumerate(conv["turns"]):
            if i % 2 == 0:
                ques.append(turn["text"].lower().strip('?').strip())
            else:
                ans.append(turn)
        ques = ques[:len(ans)]
    ...
    for i, rewrite in enumerate(tqdm(all_ques)):
        ...
        if ans["text"] != 'UNANSWERABLE':
            j += 1
            if idx == 0:
                dataset = 'ocoqa_train_original'
            else:
                dataset = 'ocoqa_test_original'
